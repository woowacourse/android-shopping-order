...
- [ ] 상품 목록을 서버에서 조회한다.
- [ ] 서버 응답 DTO를 도메인 모델로 변환한다.
- [ ] 데이터 로딩 전 스켈레톤 UI를 노출한다.
- [ ] 사용자 인증 정보를 저장한다.
- [ ] 저장된 인증 정보를 서버 요청에 사용한다.
- [ ] Kotlinx Serialization을 사용해 JSON을 직렬화/역직렬화한다.
...
- [ ] 장바구니 상품을 선택할 수 있다.
- [ ] 선택한 상품만 주문하기 버튼을 누를 수 있다.
- [ ] 선택한 상품이 없으면 주문할 수 없다.

#### 최근 본 상품 기반 추천

- [ ] 사용자가 최근 본 상품의 카테고리를 저장한다.
- [ ] 가장 최근에 본 상품의 카테고리를 기준으로 추천 상품을 조회한다.
- [ ] 같은 카테고리 상품을 최대 10개까지 노출한다.
- [ ] 해당 카테고리 상품이 10개 미만이면 가능한 개수만 노출한다.
- [ ] 이미 장바구니에 담긴 상품은 추천 목록에서 제외한다.

#### 추천 상품 장바구니 추가

- [ ] 추천 상품 화면에서 상품을 바로 장바구니에 추가할 수 있다.
- [ ] 추천 상품을 추가하면 함께 주문할 수 있다.

#### 테스트

- [ ] 최근 본 상품 카테고리 기준으로 추천 상품을 반환하는지 테스트한다.
- [ ] 추천 상품이 최대 10개까지만 반환되는지 테스트한다.
- [ ] 장바구니에 이미 담긴 상품은 추천되지 않는지 테스트한다.
- [ ] 장바구니에서 선택한 상품만 주문 대상이 되는지 테스트한다.
