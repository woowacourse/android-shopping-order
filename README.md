...
- [ ] `Payment` 도메인 모델에 대한 단위 테스트를 작성
- [ ] `PaymentViewModel`를 외부 의존성을 주입받는 구조로 정리하여, 격리된 환경에서 테스트 가능한 형태로 수정

### 복합 쿠폰 구조 대응
- [ ] '첫 구매자 추가 할인 + 배송비 무료'와 같은 복합 쿠폰 도입 가능성에 대비
  - 현재 `sealed interface` 기반 쿠폰 설계가 단일 효과에 머무르고 있으므로, 여러 할인 정책을 조합할 수 있는 구조(예: 컴포지트 패턴, 효과 리스트 기반 모델 등)로 확장
