...
- [ ] 결제 완료 시 상품 목록으로 이동한다.

### 요금 / 배송

- [ ] 배송비는 기본 3,000원이다.
- [ ] 무료 배송 쿠폰 적용 시 배송비 0원 처리

### 쿠폰 (조회·검증·적용)

- [x] 쿠폰 도메인 모델, DTO 매핑, 데이터소스/레포지토리 구현
- [x] 쿠폰 할인 계산기(CouponCalculator) 구현 
- [ ] 결제 화면에서 적용 가능한 쿠폰을 조회하고 1개만 적용할 수 있다.
- [ ] 쿠폰 적용은 만료일, 최소주문금액, 시간조건 등으로 검증한다.
- [ ] 쿠폰 적용 결과는 결제 완료 시에도 쿠폰 데이터 자체는 사라지지 않는다.

### 알림 및 설정

- [ ] 설정에서 미결제 알림 기능을 On/Off 할 수 있다.
- [ ] 결제 화면에 진입하면 5분 후 알림을 예약한다.
- [ ] 결제를 완료하거나 결제 화면으로 다시 진입하면 예약된 알림을 취소한다.
- [ ] 5분 안에 결제하지 않으면 알림을 노출한다.
- [ ] 알림을 클릭하면 결제 화면으로 이동한다.

### API / 데이터 동작 상세

- [ ] 주문 완료 시 장바구니에서 주문된 상품이 초기화된다.
- [ ] 쿠폰은 주문 완료 후에도 사라지지 않는다.
