...
- **상품 추천 알고리즘**
    - [ ] 최근 본 상품의 카테고리 분석 로직 구현
    - [ ] 최대 10개 노출 제한 및 카테고리 상품 매칭 로직
- **추천 상품 인터랙션**
    - [x] 장바구니 내 추천 상품 섹션 UI (LazyRow)
    - [ ] 추천 상품 즉시 장바구니 추가 기능 구현
- **단위 테스트 작성**
    - [ ] 선택 주문 로직 테스트 작성
    - [ ] 추천 알고리즘 단위 테스트 작성
...
- **RecommendationViewModel**
  - [ ] 체크한 주문의 id를 주입받는다
  - [ ] 주문할 상품의 총 가격을 주입받는다
  - [ ] 가장 마지막으로 조회한 상품의 id를 조회한다
  - [ ] 조회한 아이디로 해당 상품의 카테고리를 조회한다
  - [ ] 서버에서 카테고리 별 상품 리스트를 조회한다
  - [ ] 추천 상품을 바로 장바구니에 추가할 수 있다
  - [ ] 추천 상품을 장바구니에 추가하면 총 가격에 반영된다