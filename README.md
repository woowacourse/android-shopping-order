...
- [ ] 배송비 3,000원을 적용한다.
- [ ] 적용 가능한 쿠폰을 조회하고 적용한다.
    - [ ] 각 쿠폰에 대한 할인이 정확히 적용되어야 한다.
    - 쿠폰은 1개만 적용 가능
- [ ] 결제하기 버튼 클릭 시 최종 주문 완료

### 알림

- [ ] 알림 권한(POST_NOTIFICATIONS) 요청
- [ ] 설정에서 알림 기능을 On/Off
    - [ ] SharedPreferences로 설정 상태 유지
- [ ] 알림 예약/취소(AlarmManager)
    - [ ] 결제 화면 진입 시 5분 후 알림 예약
    - [ ] 결제 완료 또는 결제 화면 재진입 시 기존 알림 취소
- [ ] 알림 수신/노출
    - [ ] BroadcastReceiver를 통해 알림 수신
    - [ ] NotificationManager로 알림 노출
        - [ ] 5분 안에 결제하지 않을 시, `"아직 결제가 완료되지 않았어요"` 메시지 노출
