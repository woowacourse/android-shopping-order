...
import woowacourse.shopping.data.remote.server.dto.cart.items.*
...
import woowacourse.shopping.data.remote.server.dto.products.Content
...
private class ShoppingMockDispatcher : Dispatcher() {
    ...
    private fun count(): MockResponse =
        MockResponse()
            ...
            .setBody(json.encodeToString(Quantity(repository.getCartItemsCount())))

    private fun addCartItem(request: RecordedRequest): MockResponse {
        val payload = json.decodeFromString<PostCartRequest>(request.body.readUtf8())
    ...
    private fun updateCartItem(
        ...
        val id = url.pathSegments[1].toLongOrNull() ?: return errorResponse(HttpURLConnection.HTTP_BAD_REQUEST)
        val payload = json.decodeFromString<PatchQuantityRequest>(request.body.readUtf8())
        ...
        MockResponse()
            ...
            .setBody(
                when (body) {
                    ...
                    is CartItemsResponse -> json.encodeToString(body)
...
private class MockShoppingRepository {
    ...
    fun getProduct(id: Long): ProductResponse? {
        val content = products.find { it.id == id } ?: return null
            ...
            cartItems.values.mapNotNull { entry ->
                products.find { it.id == entry.productId }?.let { product ->
                    ContentResponse(
                        ...
                        product =
                            woowacourse.shopping.data.remote.server.dto.cart.items.ProductResponse(
...
private fun List<Content>.toPagedResponse(
    page: Int,
    size: Int,
): ProductsResponse {
    ...
    return ProductsResponse(
        ...
        pageable =
            woowacourse.shopping.data.remote.server.dto.products.Pageable(
...
private fun List<ContentResponse>.toPagedResponse(
    page: Int,
    size: Int,
): CartItemsResponse {
    val safePage = page.coerceAtLeast(0)
    ...
    return CartItemsResponse(
        ...
        empty = pageItems.isEmpty(),
        pageable =
            woowacourse.shopping.data.remote.server.dto.cart.items.Pageable(
                offset = (safePage * safeSize).toLong(),
                pageNumber = safePage,
                pageSize = safeSize,
                paged = true,
