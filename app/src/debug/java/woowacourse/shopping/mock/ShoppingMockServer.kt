...
import woowacourse.shopping.data.network.cart.dto.Content as CartContent
import woowacourse.shopping.data.network.cart.dto.ProductDto as CartProductDto
import woowacourse.shopping.data.network.product.dto.Content as ProductContent
...
private class ShoppingMockDispatcher : Dispatcher() {
    ...
    private fun count(): MockResponse = jsonResponse(Quantity(repository.getCartItemsCount()))

    private fun addCartItem(request: RecordedRequest): MockResponse {
        val payload = json.decodeFromString<CartItemInsertDto>(request.body.readUtf8())
    ...
    private fun order(request: RecordedRequest): MockResponse {
        val cartItemIds = json.decodeFromString<List<Long>>(request.body.readUtf8())
        repository.order(cartItemIds)
        // OrderService returns ProductResponse, so return an empty one.
        return jsonResponse(
            ProductResponse(
                content = emptyList(),
                empty = true,
        ...
        MockResponse()
            ...
            .setBody(
                when (body) {
                    ...
                    is ProductDto -> json.encodeToString(body)
                    is CartItemDto -> json.encodeToString(body)
...
private class MockShoppingRepository {
    ...
    fun getProducts(
        ...
        return filtered.toProductPagedResponse(page, size)
    }

    @Synchronized
    fun getProduct(id: Long): ProductDto? = products.find { it.id == id }
    ...
    fun getCartItems(
        ...
        return allItems.toCartPagedResponse(page, size)
    }
...
private val defaultSort = Sort(empty = true, sorted = false, unsorted = true)

private fun defaultPageable(
    page: Int,
    size: Int,
) = Pageable(
    offset = page * size,
...
private fun List<ProductDto>.toProductPagedResponse(
    page: Int,
    size: Int,
): ProductResponse {
    ...
    return ProductResponse(
        ...
        pageable = defaultPageable(safePage, safeSize),
        sort = defaultSort,
    )
}

private fun List<CartContent>.toCartPagedResponse(
    page: Int,
    size: Int,
): CartItemDto {
    val safePage = page.coerceAtLeast(0)
