...
class FakeProductRepository(private val cartRepository: CartRepository = FakeCartRepository()) :
    ProductRepository {
    ...
    override suspend fun getProductById(id: Long): Result<Product, DataError> {
        val product =
            productStubs.firstOrNull { it.id == id } ?: return Result.Error(DataError.NotFound)
        return Result.Success(product)
    }

    override suspend fun getAllRecommendProducts(category: String): Result<List<Product>, DataError> {
        val cartProductIds =
            when (val carts = cartRepository.getAllCartItems()) {
