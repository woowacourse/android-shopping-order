...
class BogoCouponTest {
    ...
    fun `동일 상품 3개 이상 구매하고 만료일 이전이면 쿠폰 사용이 가능하다`() {
        ...
        // when
        val actual = BOGO_COUPON.isAvailable(cartItems, LocalDateTime.of(2025, 6, 30, 5, 0))
    ...
    fun `만료일 이후면 쿠폰 사용이 불가능하다`() {
        ...
        // when
        val actual = BOGO_COUPON.isAvailable(cartItems, LocalDateTime.of(2025, 7, 1, 5, 0))
    ...
    fun `동일 상품 3개 이상이면 한 개의 상품 금액만큼 할인 금액을 반환한다`() {
        val cartItems: List<ShoppingCart> = listOf(createCartItem(1000_000, 3))
