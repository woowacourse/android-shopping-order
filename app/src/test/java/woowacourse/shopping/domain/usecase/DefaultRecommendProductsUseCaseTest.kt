...
class DefaultRecommendProductsUseCaseTest {
    ...
    @Test
    fun `최근 상품이 없으면, 카트에 없는 상품을 추천한다`() =
        runTest {
            // given
            val recommendSize = 3
            val totalProducts =
    ...
    @Test
    fun `최근 상품이 있으면, 카트에 없고 최근 상품의 카테고리에 해당하는 추천 상품을 가져온다`() =
        runTest {
            // given
            val recommendSize = 3
            val category = "식품"
            ...
            val totalProducts =
                listOf(
                    ...
                    fakeProduct(id = 2, category = category),
            ...
            coEvery { cartRepository.loadCart() } returns Result.success(cart)
            coEvery { productRepository.loadProducts(category, 0, any()) } returns
                Result.success(
                    totalProducts,
                )
            // when
    ...
    @Test
    fun `추천할 상품 개수보다 적을 경우, 그 개수만큼만 추천한다`() =
        runTest {
            // given
            val recommendSize = 3
            val category = "식품"
