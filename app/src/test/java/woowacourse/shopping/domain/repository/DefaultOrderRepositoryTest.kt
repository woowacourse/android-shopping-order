...
class DefaultOrderRepositoryTest {
    ...
    @Test
    fun `주문 아이템들을 저장하고 불러온다`() = runTest {
        // given setup empty cartSource and orderSource
    ...
    @Test
    fun `주문 아이템들의 총 가격의 합을 구한다`() = runTest {
        // given
        cartSource =
            FakeShoppingCartDataSource(
                cartItemResponses = cartItemDtosTestFixture(
        ...
        orderSource =
            FakeOrderDataSource(
                orderItems = orderItemsDataTestFixture(
                    dataCount = 10,
    ...
    @Test
    fun `상품 id 를 가진 주문 아이템의 개수를 업데이트한다`() = runTest {
        // given
        cartSource = FakeShoppingCartDataSource(
            cartItemResponses = listOf(
                CartItemResponse(
    ...
    @Test
    fun `상품 id 를 가진 주문 아이템의 개수를 업데이트하고 로드해서 확인한다`() = runTest {
        cartSource = FakeShoppingCartDataSource(
            cartItemResponses = listOf(
                CartItemResponse(
                    id = 101,
    ...
    @Test
    fun `주문 item 의 모든 개수를 구한다`() = runTest {
        // given
        cartSource = FakeShoppingCartDataSource(
            cartItemResponses = listOf(
                CartItemResponse(
    ...
    @Test
    fun `모든 주문 아이템의 총 가격 (각 개수와 가격의 곱의 합계 ) 을 구한다`() = runTest {
        // given
        cartSource = FakeShoppingCartDataSource(
            cartItemResponses = listOf(
                CartItemResponse(
...
fun orderItemsDataTestFixture(
    ...
    List(dataCount) { index -> orderItemFixture(index) }
