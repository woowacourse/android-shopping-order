...
class ShoppingViewModelTest {
    ...
    @Test
    fun `상품 목록을 성공적으로 불러오면 products 상태가 갱신되어야 한다`() =
        runTest {
    ...
    @Test
    fun `장바구니에 상품을 추가하면 cart에 실시간으로 반영되어야 한다`() =
        runTest {
            val product =
                Product(
                    id = 1L,
    ...
    @Test
    fun `최근 본 상품에 상품을 추가하면 viewHistory의 상태가 변경된다`() =
        runTest {
            val product =
                Product(
                    id = 1L,
    ...
    @Test
    fun `특정 상품의 수량을 변경하면 장바구니에 반영되어야 한다`() =
        runTest {
            val product =
                Product(
                    id = 1L,
