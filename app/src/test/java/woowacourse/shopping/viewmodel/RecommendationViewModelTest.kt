...
class RecommendationViewModelTest {
    ...
    @Test
    fun `최근 본 상품의 카테고리의 상품 추천 목록을 불러온다`() = runTest {
        // given: 상품 목록이 주어진다
        val snack1 = Product(
            category = "과자",
            id = 1L,
        ...
        assertEquals(snack2.id, viewModel.recommendedProducts.value.products[1].id)
    }

    @Test
    fun `상품을 장바구니에 추가하면 결제할 금액에 반영된다` () = runTest {
