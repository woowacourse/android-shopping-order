...
class ProductDetailViewModelTest {
    ...
    @Test
    fun `초기 진입 시 상품 상세를 불러와 state productState 에 Success 로 노출한다`() =
        runTest {
    ...
    @Test
    fun `recentProductId 가 현재 상품과 다르면 state recentProductState 에 노출된다`() =
        runTest {
            val viewModel = newViewModel()
    ...
    @Test
    fun `recentProductId 가 현재 상품과 같으면 recentProductState 는 노출되지 않는다`() =
        runTest {
            val viewModel = newViewModel()
    ...
    @Test
    fun `increase 이벤트의 결과가 state quantity 에 반영된다`() =
        runTest {
            val viewModel = newViewModel()
    ...
    @Test
    fun `decrease 이벤트의 결과가 state quantity 에 반영된다`() =
        runTest {
            val viewModel = newViewModel()
            viewModel.initialLoading(productId = 1)
    ...
    @Test
    fun `addToCart 는 기존 수량과 현재 quantity 의 합을 CartRepository 에 반영한다`() =
        runTest {
