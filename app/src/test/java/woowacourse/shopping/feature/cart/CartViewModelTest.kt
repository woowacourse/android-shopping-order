...
class CartViewModelTest {
    ...
    @Test
    fun `초기 진입 시 장바구니 항목을 불러와 state paginatedCartContents 에 노출한다`() =
        runTest {
            val viewModel =
                newViewModel(
                    initial =
    ...
    fun `이미 존재하는 상품을 증가시키면 수량이 누적된다`() =
        runTest {
            ...
            val target =
                viewModel.uiState.value.paginatedCartContents
                    .first { it.productUiModel.id == 1L }
    ...
    @Test
    fun `state paginatedCartContents 가 비면 isEndPage 가 true 가 된다`() =
        runTest {
    ...
    @Test
    fun `외부에서 장바구니에 변경이 생기면 cart 재조회로 state 에 반영된다`() =
        runTest {
