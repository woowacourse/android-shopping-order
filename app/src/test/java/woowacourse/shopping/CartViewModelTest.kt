...
class CartViewModelTest {
    ...
    fun 마지막_페이지의_마지막_아이템_삭제시_페이지가_이전_페이지로_이동한다() {
        repeat(6) {
            fakeRepository.insert(CartProduct(Goods(id = it.toLong(), name = "item $it", price = 100, thumbnailUrl = "url"), 0))
        }
        viewModel.plusPage()
        viewModel.updatePageButtonStates()
        assertEquals(2, viewModel.page.getOrAwaitValue())

        var lastItem: CartProduct? = null
    ...
    fun insertToCart_호출시_cart_LiveData가_변경된다() {
        ...
        val expectedCart = CartProduct(goods, 2) // quantity should be 2 after insertToCart
        viewModel.insertToCart(CartProduct(goods, 1))
