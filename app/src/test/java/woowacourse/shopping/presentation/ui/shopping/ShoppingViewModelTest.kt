...
class ShoppingViewModelTest {
    ...
    @BeforeEach
    fun setUp() {
        coEvery { repository.getCartItemsCounts() } returns Result.success(10)
        viewModel = ShoppingViewModel(repository)
    }

    @Test
    fun `viewModel이 초기화되면 데이터가 20개 불러와진다`() {
        coEvery { repository.getProductsByPaging(any(), any()) } returns Result.success(LoadResult.Page(0, false, cartProducts))
    ...
    fun `viewModel에서 데이터 로드가 실패하면 Error로 상태가 변화한다`() {
        coEvery { repository.getProductsByPaging(any(), any()) } returns Result.failure(Throwable())
        viewModel.loadProductsByOffset()
        Thread.sleep(1000)
