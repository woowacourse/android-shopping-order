...
class CartViewModelTest {
    ...
    @Test
    fun `장바구니에 20개의 상품이 있으면 20개 상품을 모두 로드한다`() {
        // given
    ...
    @Test
    fun `장바구니에 담겨져 있지 않은 상품의 개수를 증가시키면 장바구니에 추가된다`() {
        // given
    ...
    @Test
    fun `product id 0을 가진 상품을 장바구니에서 삭제한다`() {
        // given
    ...
    @Test
    fun `최근 본 상품이 없는 경우 상품을 추천하지 않는다`() {
        // given
    ...
    @Test
    fun `food 상품을 가장 최근에 보고 food 상품이 5개인 경우 5개의 상품을 추천한다`() {
        // given
    ...
    @Test
    fun `food 상품을 가장 최근에 보고 food 상품이 15개인 경우 10개의 상품을 추천한다`() {
        // given
    ...
    @Test
    fun `food 상품을 가장 최근에 보고 모든 food 상품이 장바구니에 있는 경우 상품을 추천하지 않는다`() {
        // given
        val products = List(15) { product(it).copy(category = "food") }
        val recentProducts = recentProducts(1)
        setUpViewModel(
    ...
    @Test
    fun `장바구니에 상품이 3개가 있고 3개를 선택한 경우 모든 상품이 선택되었다`() {
        // given
        setUpViewModel(savedProducts = products(5), savedCartItems = cartItems(3))
        viewModel.loadAllCartItems()
    ...
    @Test
    fun `하나 이상의 상품을 선택한 경우 주문할 수 있다`() {
        // given
        setUpViewModel(savedProducts = products(5), savedCartItems = cartItems(3))
        viewModel.loadAllCartItems()
    ...
    @Test
    fun `상품을 하나도 선택하지 않은 경우 주문할 수 없다`() {
        // given
        setUpViewModel(savedProducts = products(5), savedCartItems = cartItems(3))
        viewModel.loadAllCartItems()
    ...
    @Test
    fun `상품 두개를 선택하고 주문을 하면 장바구니에서 두개의 상품이 삭제된다`() {
        // given
        setUpViewModel(savedProducts = products(5), savedCartItems = cartItems(5))
        viewModel.loadAllCartItems()
        ...
        // then
        val actual = viewModel.cartUiModels.getOrAwaitValue()
        assertThat(viewModel.isSuccessCreateOrder.getOrAwaitValue().peekContent()).isTrue
        assertThat(actual.size).isEqualTo(3)
    }

    @Test
    fun `1500원 상품 2개와 3000원 상품 1개를 선택하면 총 가격이 6000원이다`() {
        // given
    ...
    @Test
    fun `1500원 상품 2개와 3000원 상품 1개를 선택하면 총 개수가 3개다`() {
        // given
        val products = listOf(product(0).copy(price = 1500), product(1).copy(price = 3000))
        val cartItems = listOf(cartItem(0, quantity = 2), cartItem(1, quantity = 1))
        setUpViewModel(savedProducts = products, savedCartItems = cartItems)
