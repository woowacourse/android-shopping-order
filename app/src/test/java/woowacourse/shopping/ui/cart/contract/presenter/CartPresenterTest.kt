...
internal class CartPresenterTest {
    ...
    fun `장바구니에 담긴 상품을 보여준다`() {
        ...
        every { cartRepository.getAllProductInCart(any()) } answers {
            val callback: (List<CartProduct>) -> Unit = arg(0)
            callback(emptyList())
        }
        every { cartRepository.getSubList(any(), any(), any()) } answers {
            val callback: (List<CartProduct>) -> Unit = arg(0)
            callback(emptyList())
        }
        ...
        assertEquals(expectedCartUIModel, slot.captured)
    ...
    fun `장바구니에 담긴 상품을 삭제한다`() {
        ...
        every { cartRepository.remove(idToRemove, any()) } answers {
            val callback: (Unit) -> Unit = arg(0)
            callback(Unit)
        }
    ...
    fun `상세 페이지로 이동한다`() {
        ...
        every { cartRepository.findById(any(), any()) } answers {
            val callback: (CartProduct) -> Unit = arg(0)
            callback(CartProduct(1, 1, fakeProduct))
        }
