...
class ProductContentsViewModelTest {
    ...
    @Test
    fun `viewModel을 생성하면, 초기에 20개의 상품이 load 된다`() = runTest {
    ...
    @Test
    fun `상품을 가져올 때, 한번에 20개씩 가져온다`() = runTest {
        // when
    ...
    @Test
    fun `상품을 불러올 때, 상품의 불러오기 전에는 로딩 상태가 된다`() = runTest {
        // when
    ...
    @Test
    fun `장바구니에 해당 상품이 없을 때, 상품을 새로 추가하면 해당 상품의 quantity가 1이 된다`() = runTest {
        // when
    ...
    @Test
    fun `장바구니에 상품의 개수가 2개일 때, 개수를 추가하면 3개가 된다`() = runTest {
        // when
