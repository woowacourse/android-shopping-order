...
class OrderViewModelTest {
    ...
    fun setUp() {
        ...
        productsRecommendationRepository = CategoryBasedProductRecommendationRepository(
            productSource,
            cartSource,
            historySource,
        )
    ...
    @Test
    fun `뷰모델은 데이터를 불러올 때 추천 상품들을 불러온다`() = runTest {
        // given
        productSource = FakeProductDataSource(
    ...
    @Test
    fun `뷰모델이 데이터를 불러올 때 주문 상품의 총 가격을 계산한다`() = runTest {
        // given
        orderSource = FakeOrderDataSource(
    ...
    @Test
    fun `뷰모델이 데이터를 불러올 때 주문 상품의 총 개수를 센다`() = runTest {
        // given
        orderSource = FakeOrderDataSource(
            orderItems = orderItemsDataTestFixture(
                dataCount = 3,
    ...
    @Test
    fun `추천 상품을 추가하면 주문 상품의 총 개수가 1 증가한다`() = runTest {
        // given
        orderSource = FakeOrderDataSource(
            orderItems = orderItemsDataTestFixture(
                dataCount = 3,
    ...
    @Test
    fun `추천 상품을 추가하면 주문 상품의 총 가격의 합도 변경된다`() = runTest {
        // given
        orderSource = FakeOrderDataSource(
            orderItems = orderItemsDataTestFixture(
                dataCount = 3,
