...
class ProductsViewModelTest {
    ...
    fun `장바구니에 담겨지지 않은 상품의 개수를 증가시키면 장바구니에 담긴다`() {
        ...
        cartRepository.findByProductId(0) {
            it.onSuccess { cartItem ->
                assertThat(cartItem).isNotNull
                assertThat(cartItem?.productId).isEqualTo(0)
    ...
    @Test
    fun `최근 본 상품이 5개인 경우 5개의 최근 본 상품을 불러온다`() {
        val products = products(5)
    ...
    @Test
    fun `최근 본 상품이 15개인 경우 10개의 최근 본 상품을 불러온다`() {
    ...
    @Test
    fun `각 상품 2개와 3개를 장바구니에 담으면 총 상품의 수량이 5개이다`() {
        val products = products(10)
