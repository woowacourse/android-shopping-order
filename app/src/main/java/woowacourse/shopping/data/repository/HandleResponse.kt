...
fun <T : Any?, S : Any?> handleError(error: ApiResult.Error<S>): Error<T> =
    when (error.code) {
...
inline fun <T : Any?> ApiResponse<T>.onSuccess(executable: (T) -> Unit): ApiResponse<T> =
    apply {
...
inline fun <T : Any?> ApiResponse<T>.onError(executable: (Error<Nothing>) -> Unit): ApiResponse<T> =
    apply {
        if (this is Error) {
...
inline fun <T : Any?> Error<T>.toErrorNothing(): Error<Nothing> =
    when (this) {
...
inline fun <T : Any?> ApiResponse<T>.onException(executable: (ApiResponse.Exception<Throwable>) -> Unit): ApiResponse<T> =
    apply {
