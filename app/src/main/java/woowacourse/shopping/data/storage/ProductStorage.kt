...
object ProductStorage {
    private val productResponses = mutableMapOf<Long, ProductResponse>()
    private val ProductResponsesValues get() = productResponses.values.toList()
    ...
    private fun initialize() {
        listOf(
            ...
            productResponses[it.id] = it
        }
    }

    operator fun get(id: Long): ProductResponse = productResponses[id] ?: throw IllegalArgumentException()

    fun getProductsById(productIds: List<Long>): List<ProductResponse> {
        return productIds.mapNotNull { productResponses[it] }
    }
    ...
    fun singlePage(
        ...
        val endIndex = minOf(toIndex, productResponses.size)

        if (fromIndex >= productResponses.size || fromIndex < 0) {
            return ProductPageResponse(emptyList(), false)
        }

        val result = ProductResponsesValues.subList(fromIndex, endIndex)
        val hasNextPage = endIndex < productResponses.size

        return ProductPageResponse(result, hasNextPage)
    }
}
