    ...
    private fun getAvailableCoupons() {
        viewModelScope.launch {
            ...
            if (orderedPrice > 50_000) {
                allCoupons.find { it.code == "FREESHIPPING" }?.let { result.add(it) }
            }

            if (orderedPrice > 100_000) {
                allCoupons.find { it.code == "FIXED5000" }?.let { result.add(it) }
            }

            val hasBulkItem = orderedCarts.any { it.quantity >= 3 }
            if (hasBulkItem) {
                allCoupons.find { it.code == "BOGO" }?.let { result.add(it) }
            }
            ...
            if (isDawnTime) {
                allCoupons.find { it.code == "MIRACLESALE" }?.let { result.add(it) }
            }
